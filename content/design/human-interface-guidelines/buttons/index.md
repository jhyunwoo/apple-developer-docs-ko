---
route: /design/human-interface-guidelines/buttons
source_url: https://developer.apple.com/design/human-interface-guidelines/buttons
source_locale: ko-KR
section: hig
content_type: article
title: 버튼
original_title: Buttons
source_hash: 70538a48120414e95af0e0f9999dd6b9d3008464baf5745b10ed9c5b546479d5
canonical_source: official-ko
last_crawled_at: '2026-03-13T15:04:05+00:00'
...
![가로로 정렬된 두 개의 스타일화된 버튼 모양이 표시되어 있음. 여섯 가지 색상으로 된 기존 Apple 로고의 빨간색을 은은하게 반영하는 빨간색 색조가 이미지에 적용됨.](https://developer.apple.com)
...
**버튼 레이블 및 콘텐츠 레이어 배경에 유사한 색상을 적용하지 마십시오.** 앱의 콘텐츠 레이어에 이미 밝고 다채로운 콘텐츠가 있는 경우, 가급적 기본 모노크롬 모양의 버튼 레이블을 사용하십시오. Liquid Glass 제어 항목의 레이블 및 제어 항목 아래의 콘텐츠에 비슷한 색상을 사용하면 색상 대비가 낮아질 수 있습니다. 자세한 지침을 보려면 [색상](https://developer.apple.com/kr/design/human-interface-guidelines/color#Liquid-Glass-color)의 내용을 참조하십시오.
...
:::note 참고
macOS 및 visionOS에서 시스템은 사람들이 버튼 위로 잠시 포인터를 가져가면 툴팁을 표시합니다. 툴팁은 버튼의 기능을 설명하는 짧은 문구를 표시합니다. 지침을 보려면 [도움말 제공하기](https://developer.apple.com/kr/design/human-interface-guidelines/offering-help)의 내용을 참조하십시오.
:::

**익숙한 동작에는 익숙한 아이콘을 연관 지어 사용하도록 하십시오.** 예를 들어, 사람들은 `square.and.arrow.up` 기호가 포함된 버튼이 공유 관련 활동을 수행하는 데 도움이 될 것이라고 예측할 수 있습니다. 버튼에 아이콘을 사용하는 것이 적합한 경우, 기존 또는 사용자화된 [SF Symbols](https://developer.apple.com/kr/design/human-interface-guidelines/sf-symbols)를 사용하는 것을 고려하십시오. 일반적인 동작을 나타내는 기호 목록은 [아이콘](https://developer.apple.com/kr/design/human-interface-guidelines/icons#Standard-icons)의 내용을 참조하십시오.
...
**윈도우 프레임이 아닌 보기에서 정사각형 버튼을 사용하십시오.** 정사각형 버튼은 도구 막대 또는 상태 막대에서 사용하기 위해 설계되지 않았습니다. [toolbar](https://developer.apple.com/kr/design/human-interface-guidelines/toolbars)에 버튼이 필요한 경우 도구 막대 항목을 사용하십시오.
...
:::note 참고
visionOS에서 버튼은 사용자 설정 호버 효과를 지원하지 않습니다.
:::
...
| --- | --- | --- | --- | --- | --- |
| 원형 | ![사용 가능 여부를 나타내는 체크 표시.](https://developer.apple.com) | ![사용 가능 여부를 나타내는 체크 표시.](https://developer.apple.com) | ![사용 가능 여부를 나타내는 체크 표시.](https://developer.apple.com) | ![사용 가능 여부를 나타내는 체크 표시.](https://developer.apple.com) | ![사용 가능 여부를 나타내는 체크 표시.](https://developer.apple.com) |
| 캡슐형(텍스트만) |  | ![사용 가능 여부를 나타내는 체크 표시.](https://developer.apple.com) | ![사용 가능 여부를 나타내는 체크 표시.](https://developer.apple.com) | ![사용 가능 여부를 나타내는 체크 표시.](https://developer.apple.com) |  |
| 캡슐형(텍스트 및 아이콘) |  |  | ![사용 가능 여부를 나타내는 체크 표시.](https://developer.apple.com) | ![사용 가능 여부를 나타내는 체크 표시.](https://developer.apple.com) |  |
| 모서리가 둥근 직사각형 |  | ![사용 가능 여부를 나타내는 체크 표시.](https://developer.apple.com) | ![사용 가능 여부를 나타내는 체크 표시.](https://developer.apple.com) | ![사용 가능 여부를 나타내는 체크 표시.](https://developer.apple.com) |  |
...
- 버튼이 유리 [윈도우](https://developer.apple.com/kr/design/human-interface-guidelines/windows#visionOS) 상단에 나타나는 경우 버튼의 배경으로 [thin](https://developer.apple.com/documentation/SwiftUI/Material/thin) 머티리얼을 사용하십시오.
- 버튼이 공간에서 떠 있는 채로 나타나는 경우 해당 배경에 [머티리얼](https://developer.apple.com/kr/design/human-interface-guidelines/materials#visionOS)을 사용하십시오.
...
![Apple Watch의 화면을 나타내는 일러스트로, 캡슐 모양의 기본 및 보조 버튼이 포함됨.](https://developer.apple.com)

**도구 막대를 사용하여 모서리에 버튼을 배치하십시오.** 시스템은 도구 막대 버튼과 부합하도록 시간 및 제목을 자동으로 이동합니다. 또한 도구 막대 버튼에도 [머티리얼](https://developer.apple.com/kr/design/human-interface-guidelines/materials#Liquid-Glass) 스타일을 적용하여, 그 아래의 콘텐츠와 명확한 시각적 구분을 제공합니다.

![상단의 앞쪽 및 뒤쪽 모서리에 도구 막대 버튼뿐만 아니라 화면 하단에서 세 개의 도구 막대 버튼을 보여주는 일러스트.](https://developer.apple.com)
...
[개인정보 보호](https://developer.apple.com/kr/design/human-interface-guidelines/privacy#Location-button)
