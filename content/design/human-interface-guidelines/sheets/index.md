---
route: /design/human-interface-guidelines/sheets
source_url: https://developer.apple.com/design/human-interface-guidelines/sheets
source_locale: ko-KR
section: hig
content_type: article
title: 시트
original_title: Sheets
source_hash: fed33c6b49c4962d3c6ebb8b4393d833144b977d0315f866b6d985afb4d4ac08
canonical_source: official-ko
last_crawled_at: '2026-03-13T15:08:44+00:00'
last_translated_at: '2026-03-14T00:08:41+09:00'
---
...
![윈도우 상단에서 아래로 펼쳐지는 시트의 스타일화된 모양이 표시되어 있음. 여섯 가지 색상으로 된 기존 Apple 로고의 빨간색을 은은하게 반영하는 빨간색 색조가 이미지에 적용됨.](https://developer.apple.com)

기본적으로 시트는 *모달*이며, 사람들이 시트를 해제할 때까지 상위 보기와 상호작용할 수 없게 만드는 대상 환경을 표시합니다(자세한 모달 표시 방식을 보려면 [모달 형식](https://developer.apple.com/kr/design/human-interface-guidelines/modality)의 내용 참조). 모달 시트는 사람들에게 특정 정보를 요청하거나, 상위 보기로 돌아가기 전에 사람들이 완료할 수 있는 간단한 작업을 표시하는 데 유용합니다. 예를 들어, 시트는 사람들이 파일 첨부, 이동하거나 저장할 위치 선택, 선택 항목의 포맷 지정 등의 동작을 완료하는 데 필요한 정보를 제공하도록 할 수 있습니다.

macOS, visionOS 및 watchOS에서는 시트가 항상 모달이지만, iOS 및 iPadOS에서는 시트가 비모달일 수도 있습니다. 비모달 시트가 화면상에 있으면 사람들은 해당 기능을 사용하여 시트를 해제하지 않고 상위 보기의 현재 작업에 직접 영향을 미칠 수 있습니다. 예를 들어, iPhone 및 iPad용 메모 앱은 비모달 시트를 사용하여 사람들이 메모를 편집할 때 다양한 텍스트 선택 항목에 서로 다른 포맷을 적용할 수 있습니다.

## 모범 사례

**시트를 사용하여 간단한 콘텐츠 또는 작업을 표시하십시오.** 시트는 상위 보기의 일부를 계속 표시하여 사람들이 시트와 상호작용할 때 원래 맥락을 유지할 수 있습니다.
...
**주요 작업에 영향을 미치는 추가 항목을 상위 보기에 표시하려면 비모달 보기를 사용하십시오.** 사람들이 메인 윈도우와 계속 상호작용하는 동안 필요한 정보와 동작에 접근할 수 있게 하려면 visionOS에서 [Split View](https://developer.apple.com/kr/design/human-interface-guidelines/split-views)를 사용하거나 macOS에서 [패널](https://developer.apple.com/kr/design/human-interface-guidelines/panels)을 사용하는 것을 고려하십시오. iOS 및 iPadOS에서는 이 작업흐름에 대해 비모달 시트를 사용할 수 있습니다. 지침을 보려면 [시트](https://developer.apple.com/kr/design/human-interface-guidelines/sheets#iOS-iPadOS)의 내용을 참조하십시오.
...
크기를 조절할 수 있는 시트는 사람들이 해당 콘텐츠를 스크롤하거나 *그래버*를 드래그할 때 확장되며, 그래버는 시트의 상단 가장자리에 나타날 수 있는 작은 가로 표시기입니다. 시트의 크기는 *디텐트*에 따라 조절되며, 디텐트는 시트가 자연스럽게 표시될 때의 특정 높이입니다. iPhone용으로 디자인된 디텐트는 시트가 자연스럽게 표시될 때의 특정 높이를 지정합니다. 시스템은 다음과 같이 두 개의 디텐트를 정의합니다. *대형*은 완전히 확장된 시트의 높이이고 *중형*은 완전히 확장된 높이의 절반 정도입니다.
...
**사람들의 예상대로 완료 및 취소 버튼을 배치하십시오.** 일반적으로, 완료 또는 닫기 버튼은 시트의 오른쪽 상단 모서리(왼쪽에서 오른쪽으로 적용된 레이아웃)에 포함됩니다. 취소 버튼은 시트의 왼쪽 상단 모서리에 포함됩니다.

취소 버튼이 오른쪽 상단에 포함된 추가 하위 보기가 있는 시트의 경우는 예외입니다. 이렇게 되면 첫 번째 이후 페이지의 왼쪽 상단에 뒤로 버튼을 표시할 공간을 제공합니다. 탐색 흐름이 끝날 때 취소 버튼을 완료 버튼으로 대치하십시오.
...
![메모 앱의 스크린샷. 배경으로 흐리게 표시된 메모 앱 문서가 있고 그 위 중앙에는 ‘메모 앱의 새로운 기능’ 시트가 표시됨.](https://developer.apple.com)
...
**사람들의 예상대로 시트의 닫기 버튼을 배치하십시오.** 사람들은 완료, 확인 및 취소 등 시트를 해제하는 모든 버튼이 보기 하단의 뒤쪽 모서리에 있을 것으로 기대합니다.
...
**시트가 윈도우의 하단 가장자리에서 나타나도록 표시하지 마십시오.** 사람들이 시트를 볼 수 있게 하려면 [공간 레이아웃](https://developer.apple.com/kr/design/human-interface-guidelines/spatial-layout#Field-of-view) 중앙에 배치하는 것이 좋습니다.
...
![Apple Watch에 기본 동작 버튼과 기본 취소 버튼이 있는 시트 스크린샷.](https://developer.apple.com)
...
**앱에 적합한 경우에만 해제 제어기의 기본 레이블을 변경하십시오.** 기본적으로 시트에는 왼쪽 상단 모서리에 원형 취소 버튼이 표시됩니다. 시트를 통해 사람들이 앱 동작이나 데이터를 변경할 수 있는 경우 이 버튼을 사용하십시오. 시트에서 작업을 활성화하지 않고 단순히 정보를 표시하는 경우, 표준 완료 버튼을 대신 사용하십시오. [도구 막대](https://developer.apple.com/kr/design/human-interface-guidelines/toolbars)를 사용하여 여러 버튼을 표시할 수 있습니다.

![Apple Watch에 표준 체크 표시 완료 버튼이 있는 시트를 표시하는 시계 스크린샷.](https://developer.apple.com)

**기본 레이블을 변경하는 경우, 동작을 나타낼 때 SF Symbols를 우선적으로 사용하십시오.** 시트가 계층적 탐색 인터페이스의 일부라고 사람들이 오해하게 할 수 있는 레이블을 사용하지 마십시오. 또한 상단의 앞쪽 모서리의 텍스트가 페이지 또는 앱 제목처럼 보이면 사람들이 시트를 해제하는 방법을 알 수 없습니다. 지침을 보려면 [아이콘](https://developer.apple.com/kr/design/human-interface-guidelines/icons#Standard-icons)의 내용을 참조하십시오.
