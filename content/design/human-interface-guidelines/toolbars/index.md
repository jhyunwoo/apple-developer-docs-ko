---
route: /design/human-interface-guidelines/toolbars
source_url: https://developer.apple.com/design/human-interface-guidelines/toolbars
source_locale: ko-KR
section: hig
content_type: article
title: 도구 막대
original_title: Toolbars
source_hash: 07d788bef166ba90765310899da934b75d037c8d3efee44af2c914e73870a587
canonical_source: official-ko
last_crawled_at: '2026-03-13T15:05:59+00:00'
...
![앞쪽 가장자리에 뒤로 제어기가 있고 뒤쪽 가장자리에 작성, 공유, 더 보기 메뉴가 있는 도구 막대의 스타일화된 모양. 여섯 가지 색상으로 된 기존 Apple 로고의 빨간색을 은은하게 반영하는 빨간색 색조가 이미지에 적용됨.](https://developer.apple.com)
...
:::note 참고
항목이 더 이상 화면에 맞지 않으면 시스템은 macOS 또는 iPadOS에서 초과 메뉴를 자동으로 추가합니다. 초과 메뉴를 수동으로 추가하지 말고, 도구 막대 항목이 기본적으로 초과되도록 만드는 레이아웃은 피하십시오.
:::
...
**도구 막대 항목 레이블 및 콘텐츠 레이어 배경에 비슷한 색상을 가급적 적용하지 마십시오.** 앱의 콘텐츠 레이어에 이미 밝고 화려한 콘텐츠가 있다면, 도구 막대는 기본 모노크롬 디자인을 사용하는 것이 좋습니다. Liquid Glass 제어 항목의 레이블 및 제어 항목 아래의 콘텐츠에 비슷한 색상을 사용하면 색상 대비가 낮아질 수 있습니다. 자세한 지침을 보려면 [색상](https://developer.apple.com/kr/design/human-interface-guidelines/color#Liquid-Glass-color)의 내용을 참조하십시오.
...
**도구 막대에서 구분 제어기를 사용하지 마십시오.** 구분 제어기를 사용하면 사람들이 각 상황 간에 전환할 수 있지만 도구 막대의 동작은 현재 보기에만 적용됩니다. 지침을 보려면 [구분 제어기](https://developer.apple.com/kr/design/human-interface-guidelines/segmented-controls)의 내용을 참조하십시오.
...
**각 제어기의 의미가 명확한지 확인하십시오.** 사람들이 도구 막대 항목의 기능을 이해하기 위해 추측하거나 실험하지 않도록 하십시오. *편집*과 같이 기호로 잘 표현되지 않는 동작을 제외하고는, 텍스트 대신 간단하고 알아볼 수 있는 기호를 항목에 우선적으로 사용하십시오. 자주 쓰는 동작을 나타내는 기호에 대한 지침을 보려면 [아이콘](https://developer.apple.com/kr/design/human-interface-guidelines/icons#Standard-icons)의 내용을 참조하십시오.
...
![앞쪽에는 필터 버튼이 있고 뒤쪽에는 완료 버튼이 있는 두 개의 도구 막대 항목 일러스트. 해당 버튼은 그룹화되지 않고, 완료 버튼에는 기본 동작임을 나타내기 위해 부각 스타일이 적용됨.](https://developer.apple.com)
...
![iPad의 Freeform 앱에 있는 상단 도구 막대의 다이어그램. 콜아웃은 도구 막대의 앞쪽 가장자리, 중앙 영역 및 뒤쪽 가장자리에 있는 항목 그룹의 위치를 나타냄.](https://developer.apple.com)
...
![도구 막대 및 윈도우 프레임의 위치를 표시하는 콜아웃이 있는 macOS의 Finder 윈도우 다이어그램.](https://developer.apple.com)
...
![visionOS에서 메모 앱 윈도우 하단을 따라 표시된 도구 막대 스크린샷.](https://developer.apple.com)
...
![visionOS의 도구 막대 스크린샷.](https://developer.apple.com)
