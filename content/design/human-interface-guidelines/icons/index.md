---
route: /design/human-interface-guidelines/icons
source_url: https://developer.apple.com/design/human-interface-guidelines/icons
source_locale: ko-KR
section: hig
content_type: article
title: 아이콘
original_title: Icons
source_hash: f174bd678e4e1a38de72e08939897080bd153b365437f6a05cd11f000a12b917
canonical_source: official-ko
last_crawled_at: '2026-03-13T14:52:53+00:00'
...
![Command 키 아이콘의 스케치. 이미지에 직사각형 및 원형 그리드 선이 겹쳐져 있고, 여섯 가지 색상으로 된 기존 Apple 로고의 노란색을 은은하게 반영하는 노란색 색조가 적용됨.](https://developer.apple.com)
...
![한 행에 네 개의 글리프가 있는 다이어그램. 글리프는 왼쪽부터 카메라, 하트, 봉투, 알람 시계임. 두 개의 수평 파선이 행의 하단과 상단을 표시하고, 한 개의 빨간색 수평선이 중간을 표시함. 네 개의 모든 글리프는 검은색이고 일부는 안에 흰색의 세부 선이 있음. 알람 시계의 시각적 무게감이 더 가벼워서 다른 글리프와 균형을 이루려면 더 높은 높이가 필요하기 때문에 알람 시계의 일부가 상단의 파선 위로 확장되어 있음.](https://developer.apple.com)

![위에 표시된 것과 동일한 네 개의 글리프가 있고, 동일한 두 개의 수평 파선이 행의 하단과 상단에 있으며 한 개의 빨간색 수평선이 중간에 있는 다이어그램. 이 다이어그램에서 네 개의 모든 글리프는 회색이며, 모든 선이 동일한 굵기임을 강조하기 위해 안에 있는 세부 선이 검은색으로 되어 있음.](https://developer.apple.com)
...
![검은색 원반 안에 흰색 화살표가 있고 그 아래의 흰색 수평 직선을 향하고 있는 이미지 두 개. 오른쪽의 이미지에는 수평으로 된 두 개의 분홍색 막대가 글리프 상단과 원반 상단 사이에 하나, 글리프 하단과 원반 하단 사이에 하나씩 있으며 글리프가 원반 안에서 기하학적으로 중앙 정렬되어 있음을 나타냄.](https://developer.apple.com)
...
![검은색 원반 안에 흰색 화살표가 있고 그 아래의 흰색 수평 직선을 향하고 있는 이미지 두 개. 왼쪽 이미지는 이전 일러스트와 동일한 위치에 두 개의 분홍색 수평 막대를 포함하고 있지만 글리프가 몇 픽셀 위로 이동되어 있음. 오른쪽 이미지에는 글리프 위로 오버레이된 분홍색 직사각형이 패딩 영역을 나타내고 있으며 글리프 아래에 픽셀이 추가로 포함되어 있음.](https://developer.apple.com)

시각적으로 중앙 정렬하기 위해 조절하는 값은 매우 작지만 앱의 모양에 큰 영향을 줄 수 있습니다.

![검은색 원반 안에 흰색 화살표가 있고 그 아래의 흰색 수평 직선을 향하고 있는 이미지 두 개. 왼쪽의 글리프는 기하학적으로 중앙 정렬되고 오른쪽의 글리프는 시각적으로 중앙 정렬됨.](https://developer.apple.com)
...
![배경을 공유하는 두 개의 도구 막대 버튼 이미지. 왼쪽 버튼은 선택 상태의 필터 아이콘을 표시하며, 배경에 파란색 색조 색상을 사용함. 오른쪽 버튼은 미선택 상태의 더 보기 아이콘을 표시하며, 도구 막대 버튼에 기본 모양을 사용함.](https://developer.apple.com)
...
| 동작 | 아이콘 | 기호 이름 |
| --- | --- | --- |
| 오려두기 | ![가위가 표시된 아이콘.](https://developer.apple.com) | `scissors` |
| 복사하기 | ![두 개의 문서 복사본이 표시된 아이콘.](https://developer.apple.com) | `document.on.document` |
| 붙여넣기 | ![클립보드 앞에 있는 문서가 표시된 아이콘.](https://developer.apple.com) | `document.on.clipboard` |
| 완료 | ![체크표시가 표시된 아이콘.](https://developer.apple.com) | `checkmark ` |
| 저장 |  |  |
| 취소 | ![X가 표시된 아이콘.](https://developer.apple.com) | `xmark` |
| 닫기 |  |  |
| 삭제 | ![휴지통이 표시된 아이콘.](https://developer.apple.com) | `trash` |
| 실행 취소 | ![왼쪽 상단으로 꺾인 화살표가 표시된 아이콘.](https://developer.apple.com) | `arrow.uturn.backward` |
| 실행 복귀 | ![오른쪽 상단으로 꺾인 화살표가 표시된 아이콘.](https://developer.apple.com) | `arrow.uturn.forward` |
| 작성 | ![정사각형 위에 있는 연필이 표시된 아이콘.](https://developer.apple.com) | `square.and.pencil` |
| 복제 | ![다른 정사각형 위에 더하기 기호가 있는 정사각형이 표시된 아이콘.](https://developer.apple.com) | `plus.square.on.square` |
| 이름 변경 | ![연필이 표시된 아이콘.](https://developer.apple.com) | `pencil` |
| 다음으로 이동 | ![폴더가 표시된 아이콘.](https://developer.apple.com) | `folder` |
| 폴더 |  |  |
| 첨부 | ![종이 클립이 표시된 아이콘.](https://developer.apple.com) | `paperclip` |
| 추가 | ![더하기 기호가 표시된 아이콘.](https://developer.apple.com) | `plus` |
| 더 보기 | ![생략 부호가 표시된 아이콘.](https://developer.apple.com) | `ellipsis` |

### 선택

| 동작 | 아이콘 | 기호 이름 |
| --- | --- | --- |
| 선택 | ![원 안에 체크표시가 표시된 아이콘.](https://developer.apple.com) | `checkmark.circle` |
| 선택 해제 | ![X가 표시된 아이콘.](https://developer.apple.com) | `xmark` |
| 닫기 |  |  |
| 삭제 | ![휴지통이 표시된 아이콘.](https://developer.apple.com) | `trash` |
...
| 동작 | 아이콘 | 기호 이름 |
| --- | --- | --- |
| 위 첨자 | ![오른쪽 상단 모서리에 숫자 1이 있는 대문자 A가 표시된 아이콘.](https://developer.apple.com) | `textformat.superscript` |
| 아래 첨자 | ![오른쪽 하단 모서리에 숫자 1이 있는 대문자 A가 표시된 아이콘.](https://developer.apple.com) | `textformat.subscript` |
| 볼드체 | ![볼드체로 대문자 B가 표시된 아이콘.](https://developer.apple.com) | `bold` |
| 이탤릭체 | ![이탤릭체로 대문자 I가 표시된 아이콘.](https://developer.apple.com) | `italic` |
| 밑줄체 | ![밑줄체로 대문자 U가 표시된 아이콘.](https://developer.apple.com) | `underline` |
| 왼쪽 정렬 | ![왼쪽 가장자리에 정렬된 상태에서 너비가 각각 다른 네 개의 수평선이 쌓여 있는 아이콘.](https://developer.apple.com) | `text.alignleft` |
| 중앙 정렬 | ![중앙에 정렬된 상태에서 너비가 각각 다른 네 개의 수평선이 쌓여 있는 아이콘.](https://developer.apple.com) | `text.aligncenter` |
| 좌우 정렬 | ![너비가 동일한 네 개의 수평선이 쌓여 있는 아이콘.](https://developer.apple.com) | `text.justify` |
| 오른쪽 정렬 | ![오른쪽 가장자리에 정렬된 상태에서 너비가 각각 다른 네 개의 수평선이 쌓여 있는 아이콘.](https://developer.apple.com) | `text.alignright` |

### 검색

| 동작 | 아이콘 | 기호 이름 |
| --- | --- | --- |
| 검색 | ![돋보기가 표시된 아이콘.](https://developer.apple.com) | `magnifyingglass` |
| 찾기 | ![문서 위에 돋보기가 표시된 아이콘.](https://developer.apple.com) | `text.page.badge.magnifyingglass` |
| 찾기 및 대치 |  |  |
| 다음 찾기 |  |  |
| 이전 찾기 |  |  |
| 선택 부분으로 찾기 |  |  |
| 필터 | ![상단에서 하단으로 갈수록 너비가 줄어드는 세 개의 수평선이 쌓여 있는 모습이 표시된 아이콘.](https://developer.apple.com) | `line.3.horizontal.decrease` |

### 공유 및 내보내기

| 동작 | 아이콘 | 기호 이름 |
| --- | --- | --- |
| 공유 | ![정사각형 중앙에서 위쪽을 가리키는 화살표가 표시된 아이콘.](https://developer.apple.com) | `square.and.arrow.up` |
| 내보내기 |  |  |
| 프린트 | ![프린터가 표시된 아이콘.](https://developer.apple.com) | `printer` |

### 사용자 및 계정

| 동작 | 아이콘 | 기호 이름 |
| --- | --- | --- |
| 계정 | ![원형 윤곽선 안에 사람의 머리 및 어깨가 추상적으로 표시된 아이콘.](https://developer.apple.com) | `person.crop.circle` |
| 사용자 |  |  |
| 프로필 |  |  |
...
| 동작 | 아이콘 | 기호 이름 |
| --- | --- | --- |
| 별로예요 | ![엄지 내림 제스처를 취하는 손이 표시된 아이콘.](https://developer.apple.com) | `hand.thumbsdown` |
| 좋아요 | ![엄지 올림 제스처를 취하는 손이 표시된 아이콘.](https://developer.apple.com) | `hand.thumbsup` |

### 레이어 순서

| 동작 | 아이콘 | 기호 이름 |
| --- | --- | --- |
| 맨 앞으로 가져오기 | ![서로 겹치는 세 개의 정사각형이 쌓여 있는 아이콘으로, 상단 정사각형은 단색 채우기 스타일을 사용하고 다른 정사각형은 윤곽으로 표시되어 있음.](https://developer.apple.com) | `square.3.layers.3d.top.filled` |
| 맨 뒤로 보내기 | ![서로 겹치는 세 개의 정사각형이 쌓여 있는 아이콘으로, 하단 정사각형은 단색 채우기 스타일을 사용하고 다른 정사각형은 윤곽으로 표시되어 있음.](https://developer.apple.com) | `square.3.layers.3d.bottom.filled` |
| 앞으로 가져오기 | ![서로 겹치는 두 개의 정사각형이 쌓여 있는 아이콘으로, 상단 정사각형은 단색 채우기 스타일을 사용하고 다른 정사각형은 윤곽으로 표시되어 있음.](https://developer.apple.com) | `square.2.layers.3d.top.filled` |
| 뒤로 보내기 | ![서로 겹치는 두 개의 정사각형이 쌓여 있는 아이콘으로, 하단 정사각형은 단색 채우기 스타일을 사용하고 다른 정사각형은 윤곽으로 표시되어 있음.](https://developer.apple.com) | `square.2.layers.3d.bottom.filled` |

### 기타

| 동작 | 아이콘 | 기호 이름 |
| --- | --- | --- |
| 알람 | ![알람 시계가 표시된 아이콘.](https://developer.apple.com) | `alarm` |
| 아카이브 | ![파일 박스가 표시된 아이콘.](https://developer.apple.com) | `archivebox` |
| 캘린더 | ![캘린더가 표시된 아이콘.](https://developer.apple.com) | `calendar` |
...
![JPG 파일에 대한 미리보기 앱의 문서 아이콘 이미지.](https://developer.apple.com)
...
![분홍색 그리드와 흰색 심전도 선 위에 분홍색 하트와 HEART 단어가 표시된 사용자 설정 문서 아이콘.](https://developer.apple.com)
...
![캔버스 너비의 10%에 해당하는 파란색 여백의 경계 안쪽에 있는 분홍색 단색 하트 모양의 다이어그램.](https://developer.apple.com)
...
![SceneKit 장면 문서 아이콘의 이미지.](https://developer.apple.com)
